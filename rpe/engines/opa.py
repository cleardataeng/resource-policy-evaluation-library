...
from rpe.exceptions import NoPossibleRemediation, NoSuchEndpoint, is_retryable_exception
...
class OpenPolicyAgent(Engine):
    ...
    def _opa_request(self, path, method="GET", data=None):
        ...
        req = request.Request(
            url, data=json.dumps(data).encode("utf-8"), method=method, headers=headers
        )
